...
def analyze_stock(ticker):
    ...
    try:
        ...
        if df.empty or len(df) < 50:
            return f"Error: Stock '{ticker}' not found or insufficient data."
        ...
        name_raw = STOCK_NAMES.get(ticker, "Unknown Target")
        name = name_raw.encode().decode('unicode_escape')
        
        # Recommendations
        trend = "Neutral"
        if rsi > 70: trend = "Overbought (Sell Warning)"
        elif rsi < 30: trend = "Oversold (Buy Opportunity)"
        elif close_price > bb_upper: trend = "Above Upper BB (Short-term Pressure)"
        elif close_price < bb_lower: trend = "Below Lower BB (Short-term Support)"
        
        msg = f"Stock Diagnosis: {name} ({ticker})\n"
        msg += f"--------------------\n"
        msg += f"Price: {close_price:.2f}\n"
        msg += f"RSI: {rsi:.2f}\n"
        msg += f"Upper BB: {bb_upper:.2f}\n"
        msg += f"Lower BB: {bb_lower:.2f}\n"
        msg += f"--------------------\n"
        msg += f"Technical Suggestion: {trend}"
...
def handle_message(event):
    ...
    if user_text.isdigit() or (".TW" in user_text) or (".TWO" in user_text):
        # 先給個初步回應，避免畫面沒反應
        try:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text="🔍 Analyzing... please wait a moment."))
        except:
            pass
            
        # 執行分析並使用 push_message 傳回結果 (不受 reply_token 效期限制)
        result = analyze_stock(user_text)
        line_bot_api.push_message(user_id, TextSendMessage(text=result))
    else:
        line_bot_api.reply_message(
            event.reply_token, 
            TextSendMessage(text="Hello! Send a stock ticker (e.g., 2330) and I'll provide technical signals!")
        )
